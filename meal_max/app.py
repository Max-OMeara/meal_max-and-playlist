...
print(
    "DB_PATH from .env:", os.getenv("DB_PATH")
)  # This should print the path to the database file for testing
...
def get_all_meals() -> Response:
    ...
    Returns:
        JSON response with all meals or an error message.
    """
    try:
        app.logger.info("Retrieving all meals")
        meals = (
            kitchen_model.get_all_meals()
        )  # Ensure get_all_meals() is defined in kitchen_model
        return make_response(jsonify({"status": "success", "meals": meals}), 200)
    except Exception as e:
        app.logger.error(f"Failed to retrieve meals: {e}")
        return make_response(jsonify({"error": str(e)}), 500)
